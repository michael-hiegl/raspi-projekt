...
try:
    ...
    while channel.voltage < 2.95 and driveallowed():
        #print("Analog Value: ", channel.value, "Voltage: ", channel.voltage)
        GPIO.output(relais_up, GPIO.HIGH)
        values = [count]
        values.append(((channel.voltage-1.5)/0.015)*1.08)
    ...
    while channel.voltage > 1.5 and driveallowed():
        #print("Analog Value.: ", channel.value, "Voltage: ", channel.voltage)
        GPIO.output(relais_down, GPIO.HIGH)
        values = [count]
        values.append((channel.voltage-1.5)/0.015)
        # values.append(channel.voltage)
        writer.writerow(values)
        count = count + 1
        time.sleep(0.1)
    ...
    plt.xlabel('Way in 0,078mm steps')
